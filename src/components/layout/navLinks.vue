...
const currentRoute = (path: string) => {
  return (route.path.includes(path) && path.length > 1) || (route.path === path) ? 'bg-muted text-primary hover:text-primary' : 'text-muted-foreground'
}
</script>

<template>
  <nav class="grid gap-2 text-lg font-medium md:gap-0 md:items-start md:text-sm md:px-4">
    <RouterLink to="#" class="flex items-center gap-2 text-lg font-semibold md:hidden">
